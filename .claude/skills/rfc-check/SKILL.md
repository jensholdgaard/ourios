...
3. **Cross-reference the accepted RFCs.** For each trigger marked, grep
   ...
   first with two read-only calls — `gh api -X GET "repos/<owner>/<repo>/pulls?state=open&per_page=100" --jq '.[].number'`, then per PR `gh api -X GET repos/<owner>/<repo>/pulls/<n>/files --jq '.[].filename' | grep '^docs/rfcs/'` — and for each such RFC (named by the plan or PR, or discovered) fetch
