...
async fn soak(config: &SoakConfig, root: &Path) -> Result<SoakReport, SoakError> {
    ...
    // PROTOTYPE (RFC 0035 red): run the soak against the
    // ordered/concurrent ingest split — id assignment stays under the
    // gate, the sink emit runs on the encode pool — so the harness
    // measures Design A's achievable multiple (`1 / serial_fraction`).
    ...
    // PROTOTYPE (RFC 0035 red): drain the concurrent encode pool before
    // the final sample + drain sweep, so every acked record has reached
    // the sink (and the D2 backlog counts it) before flush_all runs. The
    // pool's bounded queue keeps this residue to a few batches per core.
