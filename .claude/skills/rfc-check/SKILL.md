...
3. **Cross-reference the accepted RFCs.** For each trigger marked, grep
   ...
   the complete file from the PR's head branch: first `gh api -X GET repos/<owner>/<repo>/pulls/<n> --jq '[.head.repo.full_name, .head.ref] | @tsv'` for the head repository and branch (a forked PR lives elsewhere), then `gh api -X GET -H "Accept: application/vnd.github.raw+json" repos/<head repo>/contents/<path>?ref=<head branch>` (without the raw media type the Contents API returns base64 in `.content`; the skill is read-only, so only the GET form is allowed) — `gh pr diff <n>` yields a patch, so use it only for patch context or a file the PR adds whole —,
