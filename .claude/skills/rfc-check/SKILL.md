...
allowed-tools: Read, Grep, Glob, Bash(git diff --stat main...HEAD), Bash(git diff main...HEAD), Bash(git diff HEAD --stat), Bash(git diff HEAD), Bash(git diff --no-index /dev/null:*), Bash(git status --porcelain --untracked-files=all), Bash(gh pr diff:*), Bash(gh api -X GET:*)
metadata:
...
invariant or §4 hazard is a contract change and needs the RFC. A
conformance fix has three outcomes (the wire-contract row and step 3
say the same): one that restores what an accepted RFC already states is
*Not required (covered by that RFC)*; one that makes behaviour match a
spec clause no accepted RFC covers is *Recommended*, with an issue naming
the clause; one that contradicts accepted text is *Required*, as an
amendment. **If unsure, assume RFC.**
...
3. **Cross-reference the accepted RFCs.** For each trigger marked, grep
   `docs/rfcs/` for the section that specifies that surface; an RFC
   the tree does not hold yet lives in an open RFC PR, so discover those
   first with two read-only calls — `gh api -X GET --paginate "repos/<owner>/<repo>/pulls?state=open&per_page=100" --jq '.[].number'`, then per PR `gh api -X GET --paginate repos/<owner>/<repo>/pulls/<n>/files --jq '.[].filename' | grep '^docs/rfcs/'`; both need `--paginate`, since each call otherwise returns one page and "no open RFC PR overlaps" is only true of a full enumeration — and for each such RFC (named by the plan or PR, or discovered) fetch
   the complete file from the PR's head branch: first `gh api -X GET repos/<owner>/<repo>/pulls/<n> --jq '[.head.repo.full_name, .head.ref] | @tsv'` for the head repository and branch (a forked PR lives elsewhere), then, with the three values in double-quoted shell variables, `gh api -X GET -H "Accept: application/vnd.github.raw+json" -f ref="$HEAD_REF" "repos/$HEAD_REPO/contents/$RFC_PATH"` (`-f ref=` passes the branch as a URL-encoded query field, so a ref with `/` or spaces is safe; without the raw media type the Contents API returns base64 in `.content`; the skill is read-only, so only the GET form is allowed) — `gh pr diff <n>` yields a patch, so use it only for patch context or a file the PR adds whole —,
   ...
   `specified` and implemented by the change under `Coordinate` with the
   `Blocked` verdict, any other overlap with a live pre-accepted RFC (a
   change on the same surface that does not implement it) under
   `Coordinate` only (an accepted
   ...
   *accepted* RFC's design text or §5 criterion — a format or algorithm
   ...
   whatever the table says, including over the wire row's conformance-fix
   exception, which applies only when no accepted text is contradicted and the verdict must name the RFC
...
5. **If the verdict is Required, say which ladder stage gates the code.**
   ...
   RFC still `drafted` or `specified` waits (implementation begins at `red`); if part of the change needs no decision, split
