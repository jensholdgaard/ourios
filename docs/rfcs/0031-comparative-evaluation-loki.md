...
load-bearing metric is **bytes read from object storage per query** —
the implementation-independent expression of the pruning thesis —
with wall-clock latency reported as practical corroboration. Result-set
equivalence, a committed and competent (non-strawman) Loki
...
### 2.3 OTLP is where the gap is widest, and it is our workload
...
the two mechanisms diverge hardest. An OTLP log record arrives with
`severity_number`, resource attributes (`service.name`, `k8s.*`), log
attributes, and — crucially — `trace_id` / `span_id`. Ourios promotes
this structure into **queryable, statistics-bearing columns
automatically** (template id at ingest per RFC 0001; severity, service,
and configured attributes as Parquet columns per RFC 0022), so
per-row-group min/max and bloom filters prune on OTLP fields *without
the operator declaring anything*. Loki's model is the inverse: an
operator must hand-pick a small set of **low-cardinality** labels, and
everything else is brute-force chunk scan. Two consequences follow that
are specific to OTLP and decide the sharpest queries:

- Where template mining does not fire (the OTel-Demo capture is heavily
  `NO_TEMPLATE` on some services — RFC 0023), Ourios still prunes on the
  **promoted OTLP attribute columns** (severity, `service.name`,
  RFC 0022). The pruning thesis on native OTLP is therefore *template
  mining + attribute promotion*, not template mining alone — a stronger,
  more honest framing than a synthetic well-templated corpus would show.
- `trace_id` is **high-cardinality by construction**, so it cannot be a
  Loki label without exploding Loki's index. Loki must brute-force scan
  to answer "show me every log line for this trace." Ourios promotes
  `trace_id` to a bloom-filtered column and prunes to the handful of row
  groups that contain it. Trace-scoped log retrieval is a core OTLP
  operator workflow and a case where the label model *structurally*
  cannot compete — so it gets its own must-win query class (L3, §3.4).
...
Loki's own query-statistics API (`Summary.bytesProcessedPerSecond`,
`totalBytesProcessed`, `execTime`), which Loki returns per query — no
instrumentation of Loki's internals is required or permitted (that
would be a fairness hazard).
...
runs in single-binary mode with the `boltdb-shipper`/`tsdb` index and
S3 chunk storage; Ourios runs its normal ingester + querier against the
same bucket. Per the established norm (`benchmarks.md` §1, and the
project's bench-on-`ci-runner`-first discipline), the first
comparative run is **indicative on `ci-runner`**; the authoritative
run is on the `baseline-8vcpu-32gib` tag and is gated on maintainer
opt-in. Neither system is co-scheduled with the other during a timed
query (they share a bucket, not a CPU): ingest both, then quiesce, then
query each in isolation with the other stopped, to remove noisy-
neighbour effects from the latency numbers.
...
full attribute/`trace_id` structure §2.3 turns on, and heterogeneous
enough (heavily `NO_TEMPLATE` on some services) to be the *honest hard
case* rather than a favourable one. This is the number the project
stands behind. **LogHub HDFS_v1** (already wired, bench-time-fetched,
~1.47 GiB) is retained as a **secondary, well-templated sanity floor**
— it reproduces the best case for template mining and anchors against
the Drain-paper corpora — but it is explicitly not the headline, and it
is non-native (plain text replayed as text-body OTLP), so it exercises
template pruning without the OTLP-attribute story.
...
The label set is frozen in the config and reviewed as the §5 RFC0031.9
gate.

### 3.4 Query taxonomy

Six query classes, each with a must-win / acknowledged-loss / floor
disposition fixed up front so the result cannot be reframed after it is
known. The three must-win classes are chosen to cover the three ways
Ourios prunes native OTLP (template id, promoted attribute columns,
high-cardinality bloom column):

| Class | Query | Ourios pruning mechanism | Disposition |
|-------|-------|--------------------------|-------------|
| **L1** | Template-exact lookup: all lines of one rare template over the full corpus | template id (RFC 0001) | **must-win** (thesis) |
| **L2** | OTLP attribute predicate: `severity ≥ ERROR AND service.name = X` over a bounded window | promoted attribute columns + Parquet stats (RFC 0022) | **must-win** (thesis) |
| **L3** | Trace correlation: every log line for one `trace_id` | high-cardinality bloom-filtered column | **must-win** (thesis, OTLP-native) |
| **L4** | Substring needle: an arbitrary literal *not* captured by a template or a promoted column (embedded in a param) | none (brute scan for both) | **acknowledged** — loss permitted, published |
| **L5** | Broad scan: all lines in a wide time range, low predicate selectivity | little prunes | **floor** — bounded, not must-win |
| **L6** | Ingest throughput: sustained OTLP lines/s to steady state | — | **parity** — within a stated factor |

L1–L3 are where the pruning thesis lives and must win on bytes-read
(§3.6); together they exercise all three OTLP pruning mechanisms, and
L3 is the one Loki *structurally* cannot answer without a full scan
(§2.3). L4 is the honest inclusion: neither template mining nor
attribute promotion helps a substring the miner folded into a
parameter, and Loki's brute-force chunk grep may match or beat Ourios
there — we publish it. L5 tests the floor (when little can be pruned,
Ourios must not be *catastrophically* worse — bounded, not required to
win). L6 checks that thesis-side query wins are not bought with an
unacceptable ingest regression.

### 3.5 Result-set equivalence (the integrity gate)

A latency or bytes comparison between two queries that return
different rows is meaningless. For every query in the taxonomy, the
harness extracts the set of matching log lines from each system, keyed
by a stable identity (`(timestamp, body-bytes)` after both systems'
round-trip), and asserts the two sets are **identical** before any
metric for that query is recorded. A mismatch fails the run
(non-zero exit, no numbers written for that class) — it means the two
queries are not asking the same question and the comparison is invalid.
This is RFC0031.1 and it gates every other `L`-scenario.
...
- **`bytes_read`** — bytes fetched from object storage to answer the
  ...
  `Summary`. **Primary gate metric.**
- **`latency_p50` / `latency_p99`** — wall-clock over N repetitions,
  warm cache, plus one **cold** reading (process/page-cache dropped)
  stated separately. Corroborating, not sole-gating (§2.4).
...
- **`ingest_throughput`** — steady-state OTLP lines/s (L6 only).
...
orchestration — is committed under
`bench/comparative/` and runnable by a third party with one command.
The Loki configuration must be a **good-faith competent** deployment:
tuned chunk target size, appropriate index period, the label set from
§3.3. The config carries a header comment inviting challenge, and the
`L`-gate results in `benchmarks.md` §9 link the exact config commit.
A benchmark whose loser's configuration cannot be inspected and
re-run is not evidence; this section is what makes the number
defensible rather than a claim.

### 3.8 `benchmarks.md` amendments

- **§1** gains Loki as a second reference system, described as above.
- **§7** gains the `L`-gate escalation: an L1, L2, or L3 **bytes-read**
  loss on the headline OTel-Demo corpus is a **pillar-level** finding
  (revisit `CLAUDE.md` §2 before further implementation), exactly as two
  failing thesis-gates are today. A must-win *latency* loss with a
  bytes-read win is a roadmap item, not an escalation. L4 (substring)
  loss is expected and never escalates. L5 beyond its floor, or an L6
  regression past its factor, is a tuning RFC.
...
They will win outright on arbitrary substring search (L4-like queries)
...
**Latency as the primary gate.** Rejected in §2.4: it confounds the
...
equivalence a hard precondition.
...
> - **Then** it extracts each system's matching lines keyed by
>   `(timestamp_unix_nanos, body_bytes)` and asserts the two sets are
>   identical
> - **And** if the sets differ, the harness records no `L`-metric for
>   that class, writes the symmetric-difference count and up to N
>   example keys to stderr, and exits non-zero
...
> - **Given** the HDFS_v1 corpus ingested into both systems and a
>   template that matches `< 0.1%` of corpus lines
...
>   `M_L1` is the committed must-win margin (§7 open question)
...
> - **And** `latency_p50`, `latency_p99` (warm) and one cold reading are
>   recorded for both systems as corroborating, non-gating numbers

> **Scenario RFC0031.3 — L2 predicate pushdown wins on bytes read**
> - **Given** the corpus ingested into both systems and the L2
>   predicate (`severity ≥ ERROR AND service = X` over a bounded window)
>   expressed equivalently in both DSLs, equivalence per RFC0031.1
>   holding
...
> - **Given** the OTel-Demo capture ingested into both systems and a
>   `trace_id` present in the corpus, with `trace_id` **not** a Loki
>   label (per the §3.3 frozen set — it is high-cardinality and
>   un-labelable), equivalence per RFC0031.1 holding
...
>   here is the strongest possible signal against the thesis

> **Scenario RFC0031.5 — L4 substring needle is measured and published, loss permitted**
> - **Given** an L4 query for a literal not captured by a template or a
...
>   suppressed L4 loss is a process violation)

> **Scenario RFC0031.6 — L5 broad scan stays within the floor**
> - **Given** an L5 low-selectivity wide-time-range query, equivalence
>   holding
> - **When** the harness runs L5 against each
> - **Then** `ourios.latency_p50 ≤ F_L5 × loki.latency_p50` where
>   `F_L5` is the committed floor factor (§7)
> - **And** exceeding the floor is a tuning-RFC signal, not a
>   pillar-level escalation

> **Scenario RFC0031.7 — L6 ingest throughput parity within a stated factor**
...
> - **Then** `ourios.ingest_throughput ≥ loki.ingest_throughput / F_L6`
>   where `F_L6` is the committed parity factor (§7)
...
> **Scenario RFC0031.8 — Storage footprint is recorded as a diagnostic, not a gate**
...
> **Scenario RFC0031.9 — The Loki configuration is committed, competent, and re-runnable**
...
> - **And** the label set is neither a single catch-all (forcing Loki
>   into a full scan) nor a high-cardinality label such as `trace_id` or
>   one-per-template (smuggling Ourios's promoted columns into Loki's
>   index) — it is the §3.3 competent-operator set, and the config
>   header states this and invites challenge
> - **And** each `L`-gate row in `benchmarks.md` §9 links the config
>   commit used to produce it

> **Scenario RFC0031.10 — Losses are published and escalation follows benchmarks.md §7**
...
>   hardware tag
> - **And** an L1, L2, or L3 bytes-read loss on the headline OTel-Demo
>   corpus is recorded as a **pillar-level** finding that pauses further
>   implementation pending a `CLAUDE.md` §2 revisit (the §7 amendment),
>   whereas a must-win *latency-only* loss with a bytes-read win is
>   recorded as a roadmap item
...
- **Equivalence harness (RFC0031.1)** — an integration test over a
  small committed fixture corpus (not the full HDFS fetch) that runs a
  DSL↔LogQL pair against a containerised Loki and the in-process
  querier and asserts set-equality of the keyed line sets; a
  deliberately mismatched pair asserts the non-zero-exit / no-write
  path. Runs in CI on the fixture; the full-corpus run is
  workflow-dispatch (below).
- **`L`-gate computation (RFC0031.2–RFC0031.8)** — unit tests over
  recorded/synthetic per-query metric inputs assert the ratio math,
  the pass/fail dispositions, and the diagnostic-vs-gating distinction
  (mirroring RFC 0006's `a1`/`c2` gate-math unit tests). The margins
  `M_L1`, `M_L2`, `M_L3`, `F_L5`, `F_L6` are configuration, so a
  calibration test pins their wiring, not their values.
- **Bytes-read metric extension (RFC0031.2/.3/.4)** — a querier test
...
- **Config-presence / re-runnability (RFC0031.9)** — a test asserts the
  committed Loki + OTLP-path configs parse and that the documented
  one-command entry point exists and references them; the anti-strawman
  label-set property is enforced by review, recorded as such.
- **Full comparative run (RFC0031.10)** — a `workflow_dispatch` job
...
authoritative comparative run has been recorded in §9 with L1, L2, and
L3 passing on the headline OTel-Demo corpus. A must-win failure does
not block `validated` in the "we didn't finish" sense — it is a
*result*, and per §5 RFC0031.10 a pillar-level one.

## 7. Open questions

- [ ] **Must-win margins `M_L1`, `M_L2`, `M_L3`.** What bytes-read ratio
  counts as "decisively wins"? Proposed starting point: `M ≥ 10` (Ourios
  reads ≤ 1/10 the bytes Loki does on the selective classes), mirroring
  B1's 10× framing but on the honest metric; L3 (trace correlation, which
  Loki must full-scan) may warrant a larger `M_L3`. Needs a calibration
  pass on a first indicative run before frozen. **Maintainer call.**
- [ ] **Floor / parity factors `F_L5`, `F_L6`.** How much slower on the
  broad scan (L5) and how much lower on ingest (L6) is tolerable before
  it is a finding? Proposed `F_L5 = 3`, `F_L6 = 2`, pending the first
  run.
- [ ] **Loki index backend.** `tsdb` (current Loki default) vs
  `boltdb-shipper`. Pick the one a competent 2026 operator would
  deploy; likely `tsdb`. Confirm against Loki's current guidance at
  implementation time.
- [ ] **OTLP → Loki path.** Loki's native OTLP endpoint vs an OTel
  Collector with the `loki` exporter. Native OTLP is the fairer
  apples-to-apples (both consume OTLP directly); confirm label
  derivation is equivalent to the frozen set either way.
- [x] **Headline corpus — DECIDED: OTel-Demo.** Ourios is an OTLP-native
  ...
  The OTel-Demo v8 capture (heterogeneous, ~79% `NO_TEMPLATE` on some
  services per RFC 0023) is the *harder* case, and taking it as the
  headline is the more honest and more differentiating choice: it is
  where OTLP-attribute promotion (RFC 0022) and the L3 trace-correlation
  advantage (§2.3) carry the thesis, not template mining alone. HDFS_v1
  is retained only as a secondary well-templated sanity floor (§3.3).
...
- [ ] **New crate vs `ourios-bench` extension.** Does the comparative
  ...
  metric addition? A new crate is a `CLAUDE.md` §7 commitment; a
  harness under `bench/` is not. Leaning `bench/` + a querier metric
  extension. **Maintainer call.**
...
- `CLAUDE.md` §1 (the existence test — "just use $X"), §2 (pillars #1
  ...
  L5), §7 (new-crate commitment, open question).
...
- **RFC 0011** — A1 demotion to diagnostic (precedent for the
  storage-footprint diagnostic disposition, RFC0031.8).
