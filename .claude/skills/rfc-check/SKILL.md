...
2. **Apply the triggers.** Grep the touched code for the surfaces below and
   ...
   | New tunable or new config semantics | `ourios-config` (`MinerConfig`), `ourios-server/src/config` | **Required**: RFC 0004 §3.2 lists the four tunables, §3.3 closes the list (any new knob in an invariant area is an invariant proposal that goes through the RFC process, not review) and §3.5 is the escalation path, with a `meta:` RFC first if it relaxes a `CLAUDE.md` §3 invariant |
...
3. **Cross-reference the accepted RFCs.** For each trigger marked, grep
   ...
   its file from that PR (`gh pr diff <n>` or `gh api` on the PR's file),
