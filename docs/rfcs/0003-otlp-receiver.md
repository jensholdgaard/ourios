...
canonicalises every `LogRecord.body` to its OTLP-canonical JSON
encoding, fans the batch out into per-tenant streams of
...
message carrying one or more `ResourceLogs`:
...
in-memory `LogsData` representation `tonic` produces, and the
two transport paths converge into a single business-logic layer.
...
- **§3.7.RFC0003.2** — Tenant resolution failure: an export
...
- **RFC0003.1** — gRPC and HTTP/protobuf transports produce the
  identical in-memory `LogsData` for a byte-equal payload.
- **RFC0003.2** — HTTP/JSON canonicalisation: a valid OTLP/JSON
...
- **RFC0003.3** — `body.kind = Structured` records carry the
...
- **RFC0003.4** — `body.kind = String` records reach the miner
  with the unwrapped string as `L_raw` (RFC 0001 §6.2 step 0).
- **RFC0003.5** — `severity_number = 0` (UNSPECIFIED) and
  `scope_name = None` records pass through to the miner without
  rejection or coalescing (RFC 0001 §6.1).
- **RFC0003.6** — `dropped_attributes_count` from the wire is
  preserved verbatim on the record (the receiver does not add
  or recompute).
- **RFC0003.7** — Transport errors (malformed protobuf, oversize
  request, invalid `Content-Type`) produce a controlled
  transport-level error response, not a panic.
...
- HTTP: `axum` on `hyper`. A single `/v1/logs` POST handler
  ...
  - `application/x-protobuf` → `prost::Message::decode` into
    the same `LogsData` type the gRPC path produces.
...
1. Receiver accepts the request and decodes to `LogsData`.
...
the client never heard about it" case, also recoverable via
client retry (idempotency keyed on the WAL record's content
hash — see `docs/hazards.md` H3).

For the MVP corpus path (no WAL yet, per `roadmap.md` §5), the
WAL steps are no-ops; the API contract is in place so the
post-MVP shipping work just lights up the durability layer
without the receiver's surface changing.
...
- **Schema URL handling**. `ResourceLogs.schema_url` and
  `ScopeLogs.schema_url` are preserved verbatim on the
  `OtlpLogRecord` (currently as part of `resource_attributes`
  and `scope_*` fields respectively); semantic interpretation
  of the schema URL is not in scope.
...
- **WAL-before-ack** (§3.4.RFC0003.1, §3.4.RFC0003.2): an
  integration test running the receiver against a mock WAL
  that explicitly records the order of (append, fsync, ack)
  events. Crash-recovery test SIGKILLs the receiver between
  fsync and ack, restarts, replays.
- **Tenant fan-out** (§3.7.RFC0003.1, §3.7.RFC0003.2):
  hand-curated multi-Resource batches, assert that the
  per-tenant streams handed to the miner contain only the
  records from each Resource. Property test on the tenant-
  derivation rule (any rule that returns `Some` for both
  Resources never produces cross-contamination).
- **Wire-decode equivalence** (RFC0003.1, RFC0003.2): a
  property test asserting that a batch of `LogsData` objects,
  serialised to gRPC + protobuf, HTTP + protobuf, and HTTP +
  JSON, round-trips through each transport and produces
  byte-identical canonical body bytes for the same logical
  records.
- **Body canonicalisation** (RFC0003.3, RFC0003.4):
...
- **Edge OTLP cases** (RFC0003.5, RFC0003.6): hand-curated
  records with `severity_number = 0`, `scope_name = None`,
  non-zero `dropped_attributes_count`. Assert pass-through
  semantics.
- **Transport-level errors** (RFC0003.7): malformed protobuf,
...
- [ ] **HTTP/JSON support — required at MVP, or only protobuf?**
  Many OTel emitters use HTTP/protobuf; HTTP/JSON is less
  common but spec-compliant. The cost of supporting JSON is
  the canonicalisation logic and an additional decode path.
  Specifying *now* is cheaper than retrofitting; deferring
  saves complexity if JSON usage is rare in target
  deployments.
