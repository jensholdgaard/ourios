...
Note what `template_id` is and is not: an Ourios-derived, tenant-local
`u64`, sitting in the same flat namespace as real OTLP fields but not
one of them, and not portable between stores (RFC 0010 drift, RFC 0007
aliases exist because of exactly that). A template's portable identity
is its string — `log.record.template` in the emerging OpenTelemetry
convention (RFC 0050).
