...
/// RFC0013.1 — a `MinedRecord` batch written and read through the `AmazonS3`
...
/// RFC0013.2 — the existing RFC 0005 / 0009 suites pass unchanged against the
...
/// RFC0013.3 — two `compact_partition` runs racing on one partition: exactly
...
/// RFC0013.4 — generation publish uses conditional PUT (`PutMode::Create` /
...
/// RFC0013.5 — operations in tenant X's context address only X's key
...
/// RFC0013.6 — with an object-storage backend, only data/audit/manifest
...
/// RFC0013.7 — an S3-compatible store (`MinIO`) configured via an endpoint
...
/// RFC0013.8 — the RFC 0005 §3.9 reader forward-compat contract (absent
